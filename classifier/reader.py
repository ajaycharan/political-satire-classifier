#Based on https://github.com/tensorflow/tensorflow/blob/master/tensorflow/models/rnn/ptb/reader.py
...
def _read_words(filepath):
    words = []
    for filename is os.listdir(filepath):
        with tf.gfile.GFile(filename, "r") as f:
            words += ntlk.word_tokenize(f.read().decode("utf-8").replace("\n", "<eos>"))
    return words
...
def chunk_producer(raw_data, batch_size, num_steps, name=None):
