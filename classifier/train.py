...
tf.flags.DEFINE_string("filter_sizes", "2, 4, 8, 16, 32", "Comma-separated filter sizes (default: '2, 4, 8, 16, 32')")
...
np.random.seed(10)
train_shuffle_indices = np.random.permutation(np.arange(len(y)))
x_shuffled = x[train_shuffle_indices]
y_shuffled = y[train_shuffle_indices]
...
with tf.Graph().as_default():
    ...
    with sess.as_default():
        ...
        def batch_dev_step(dev_batches, writer=None):
            total = 0
            correct = 0
            t_step = 0
            for dev_batch in dev_batches:
                x_dev_batch, y_dev_batch = zip(*dev_batch)
                total += len(x_dev_batch)
                feed_dict = {
                ...
                step, summaries, loss, accuracy = sess.run(
                   ...
                   feed_dict)
                t_step = step
                correct += accuracy*len(dev_batch)
                #print("{}: step {}, loss {:g}, acc {:g}".format(time_str, step, loss, accuracy))
            if total > 0:
                time_str = datetime.datetime.now().isoformat()
                print("{}: step {}, acc {:g}".format(time_str, t_step, correct/total))            
            else:
                print "Dev batches are empty"
            #if writer:
            #    writer.add_summary(summaries, step)
        ...
        for batch in batches:
            ...
            if current_step % FLAGS.evaluate_every == 0:
                dev_batches = data_helpers.batch_iter(
                    list(zip(x_dev, y_dev)), FLAGS.batch_size, num_epochs=1, shuffle=False)
                print("\nEvaluation:")
                batch_dev_step(dev_batches, writer=dev_summary_writer)
                print("")
